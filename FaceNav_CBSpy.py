...
def smile_ratio(lm):
    # Original SRS/SDD metric: mouth width / mouth height (landmarks 61, 291, 13, 14)
...
class FaceNavApp:
    def __init__(self, root):
        ...
        self.smile_thresh = tk.DoubleVar(value=1.5)
    ...
    def _load_settings(self):
        ...
        try:
            ...
            self.smile_thresh.set(s.get("smile_thresh",1.5))
    ...
    def _build_ui(self):
        ...
        sliders = [
            ...
            ("SMILE SENSITIVITY",    self.smile_thresh,  0.5,  4.0,  C["purple"],
             "Margin above your calibrated neutral — raise if triggering without smiling"),
        ...
        for label, var, lo, hi, color, hint in sliders:
            ...
            ci = tk.Frame(card, bg=C["card"], padx=18, pady=14)
            ci.pack(fill="x")
            ...
            tk.Label(srow, text=f"{lo:.2f}",
                     font=("Consolas", 9),
                     bg=C["card"], fg="white").pack(side="left")
        ...
        if ret:
            ...
            if results.face_landmarks:
                ...
                if self.calibrating:
                    ...
                    # Smile: only fire when score rises above the user's personal
                    # neutral baseline (recorded during calibration) by at least the
                    # slider margin. mw/mh at rest is ~3-5; a real smile pushes it to ~6-9.
                    # slider range 0.5-3.0 directly adds to the baseline as a raw margin.
                    baseline     = self.smile_baseline if self.smile_baseline is not None else 100.0
