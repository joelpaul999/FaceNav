...
class FaceNavApp:
    ...
    def _setup_combobox_style(self):
        ...
        self.root.option_add("*TCombobox*Listbox.background", C["card_hi"])
        self.root.option_add("*TCombobox*Listbox.foreground", C["cyan"])
        self.root.option_add("*TCombobox*Listbox.selectBackground", C["cyan_mid"])
    ...
    def _build_ui(self):
        ...
        self.btn_start = self._hud_btn_grid(ctrl_row,
            "▶   ENGAGE FACE MOUSE",
            self._toggle_tracking, C["black"], C["green_dim"], col=0)
        self._hud_btn_grid(ctrl_row, "⟳   RECALIBRATE",
            self._recalibrate, C["black"], C["cyan_dim"], col=1)
        self._hud_btn_grid(ctrl_row, "↓   COMMIT SETTINGS",
            self._save_settings, C["black"], C["purple_dim"], col=2)
        ...
        for idx, (label, var, lo, hi, color, hint, zone_fn) in enumerate(sliders):
            ...
            c_slider = tk.Canvas(srow, bg=C["bg2"], height=14,
                                 highlightthickness=1,
                                 highlightbackground=C["border"])
            c_slider.pack(side="left", fill="x", expand=True, padx=(8, 8))
            c_slider.create_rectangle(0, 0, 0, 16, fill=color, outline="", tags="fill")
            c_slider.create_rectangle(0, 0, 0, 16, outline=C["green"], width=2, tags="handle")

            def _drag(e, v=var, mn=lo, mx=hi, c=c_slider):
                w = c.winfo_width()
                if w < 10: w = 150
                pct = max(0.0, min(1.0, e.x / w))
                v.set(mn + pct * (mx - mn))

            c_slider.bind("<Button-1>", _drag)
            c_slider.bind("<B1-Motion>", _drag)
            
            c_slider.bind("<Enter>", lambda e, c=c_slider: c.config(highlightbackground=C["cyan"]))
            c_slider.bind("<Leave>", lambda e, c=c_slider: c.config(highlightbackground=C["border"]))
            ...
            def _trace(v=var, vv=val_var, zfn=zone_fn, zd=zone_dot, zl=zone_lbl, c=c_slider, mn=lo, mx=hi):
                def cb(*_):
                    val = v.get()
                    vv.set(f"{val:.2f}")
                    ztxt, zcol = zfn(val)
                    ...
                    c.itemconfig("fill", fill=zcol)
                    w = c.winfo_width()
                    if w < 10: w = 200
                    fill_w = max(0, min(w, (val - mn)/(mx - mn)*w))
                    c.coords("fill", 0, 0, fill_w, 16)
                    c.coords("handle", fill_w - 2, 0, fill_w + 2, 16)
                    c.itemconfig("handle", outline=zcol)
                v.trace_add("write", cb); cb()
                c.bind("<Configure>", lambda e: cb())
            _trace()
            ...
            if results.face_landmarks:
                ...
                if self.calibrating:
                    ...
                    if self.origin_x:
                        ...
                        # Complex reticle for origin
                        cv2.circle(overlay, (ox, oy), 15, (255, 130, 0), 1)
                        cv2.line(overlay, (ox-20, oy), (ox+20, oy), (255, 130, 0), 1)
                        cv2.line(overlay, (ox, oy-20), (ox, oy+20), (255, 130, 0), 1)
                        
                        # Nose point tracker
                        cv2.drawMarker(overlay, (int(nx), int(ny)), (0, 229, 255), markerType=cv2.MARKER_CROSS, markerSize=12, thickness=2)
                        cv2.circle(overlay, (int(nx), int(ny)), 4, (0, 255, 157), -1)
                        cv2.line(overlay, (ox, oy), (int(nx), int(ny)), (0, 255, 157), 2)

                    # Biometric bars with alpha background
                    for val, lbl, thresh, bx in [
                            (l_ear_v, "L-EYE", et, 16),
                            (r_ear_v, "R-EYE", et, 116)]:
                        cv2.rectangle(overlay, (bx, fh-28), (bx+80, fh-16), (20, 30, 50), -1)
                        bar = int(np.clip(val * 240, 0, 80))
                        clr = (80, 40, 255) if val < thresh else (0, 255, 157)
                        cv2.rectangle(overlay, (bx, fh-28), (bx+bar, fh-16), clr, -1)
                        cv2.putText(overlay, lbl, (bx, fh-32), cv2.FONT_HERSHEY_SIMPLEX, 0.35, clr, 1)

                    # Augmented HUD corners
                    margin, length, thick = 12, 24, 2
                    clr_hud = (0, 229, 255)
                    for (x, y, ddx, ddy) in [
                            (margin, margin, 1, 1),
                        ...
                        cv2.line(overlay, (x, y), (x+ddx*length, y), clr_hud, thick)
                        cv2.line(overlay, (x, y), (x, y+ddy*length), clr_hud, thick)
                        cv2.circle(overlay, (x, y), 3, (0, 255, 157), -1)
